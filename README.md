...
Сервер ожидает ответ Qwen, проверяет каждое `value` по исходному тексту и самостоятельно восстанавливает точные `start/end`, если модель ошиблась в позициях. Ответ API содержит безопасную трассировку без текста и персональных значений: `requestId`, длительность, HTTP-статус прокси, число переданных локальных кандидатов, найденных, исправленных, принятых, добавленных и отклонённых сущностей. При ошибке или таймауте Qwen обработка продолжается локальными правилами.
