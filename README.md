...
Qwen — второй автоматический детектор, который вызывается при наличии серверной настройки. Анонимайзер использует тот же профиль Qwen3.6-27B и тот же зашифрованный токен прокси, что страница `/chat`; отдельный API-ключ для него не требуется. Ключ остаётся на сервере. В модель передаются системный промпт, явная задача, формат документа, извлечённый текст и кандидаты локальных правил; бинарный TXT/DOCX/PDF и ключ восстановления не отправляются.

Сервер ожидает ответ Qwen, проверяет каждое `value` по исходному тексту и самостоятельно восстанавливает точные `start/end`, если модель ошиблась в позициях. Ответ API содержит безопасную трассировку без текста и персональных значений: `requestId`, длительность, HTTP-статус прокси, число попыток и переданных локальных кандидатов, найденных, исправленных, принятых, добавленных и отклонённых сущностей. Для временных ответов `429/502/503/504` выполняется один автоматический повтор; при повторной ошибке или локальном таймауте обработка продолжается локальными правилами.

Основной способ настройки — `/admin → Qwen Chat`. После сохранения `QWEN_PROXY_TOKEN` и параметров моделей Qwen становится доступен одновременно для `/chat` и `/anonymizer`. Переменные `QWEN_27B_*`, `QWEN_PROXY_TOKEN` и `QWEN_REQUEST_TIMEOUT_MS` из `.env.example` остаются серверным fallback для Docker и запуска без Docker.
