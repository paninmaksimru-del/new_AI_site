...
function requireQwenAuth(getDb) {
  return (req, res, next) => {
    const token = req.headers['x-auth-token'];
    if (!token) return res.status(401).json({ error: 'AUTH_REQUIRED' });
    const session = getDb().prepare('SELECT user_id FROM sessions WHERE token = ?').get(token);
...
export function setupAnonymizerQwen(app, getDb) {
  ...
  app.post('/api/anonymizer/qwen/entities', requireQwenAuth(getDb), async (req, res) => {
    const config = qwenConfig();
